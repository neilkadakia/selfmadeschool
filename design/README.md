...
- `--acc: #43DE7B` — primary accent (growth green): CTAs, marquee band, enroll section bg
...
3. **Marquee band** — full-width green band rotated −1.5° (scale 1.02), 2px ink borders, Bricolage 22px uppercase ticker "Mindset ★ Money ★ Habits ★ Big calls ★ Taxes ★ Credit ★ 401(k) ★ Negotiation ★" duplicated 2× for seamless loop.
4. **Syllabus** (`#syllabus`) — cream section, padding 110px 32px 120px, max-width 1240px. Kicker (cobalt) "The 13th Grade — Intro Course Syllabus"; H2 "Six units. Zero lectures about the mitochondria." Grid `repeat(auto-fit, minmax(320px, 1fr))`, gap 22px; six cards (min-height 240px, padding 34px 30px): each has top row (UNIT NN label 13px + lesson-count pill) and bottom block (title 27px + body 16px). Card backgrounds in order: ink, cobalt, green, amber, white (2px ink border), ink. Units: Money 101 / Tax Season, Decoded / Credit Glow-Up / Mindset Reps / Invest, Eventually / The Big Calls (copy in file).
5. **The Grades** (`#grades`) — cream continues, padding 0 32px 130px. Kicker "The Grades"; H2 "School's back on. Three grades to go." Three ink cards (min-height 380px, padding 40px 34px): giant colored numeral (13 green / 14 cobalt / 15 amber, 88px) + status pill (Start Here / Next Up / Final Year) + title + body + colored text link ("Enroll Free →" / "Join the Waitlist →" ×2).
6. **How It Works** (`#how`) — ink section, **pinned horizontal scroll**: header ("Four steps. Keep scrolling →") then a flex track of four panels `min(72vw, 760px)` wide, min-height 420px, radius 32px, gap 28px — 01 Watch (green) / 02 Do (cobalt) / 03 Flex (amber) / 04 Repeat (cream). Track translates horizontally while section is pinned (see Interactions).
7. **Receipts** (`#receipts`) — ink, top border `rgba(242,238,227,0.1)`. Four stat blocks (2px colored top border, numeral Bricolage 72px, label 15px uppercase): 40+ Lessons live now / 10 min Average lesson / $0 Tuition forever / 0 Pop quizzes. Below: centered blockquote (Bricolage 700, `clamp(26px,3.6vw,44px)`) "I came for the money stuff. The mindset unit is the one I keep re-watching." — Maya, 24 · The 13th Grade, Class of '26 (green cite).
...
3. **Three house rules** — ink section, three cards (min-height 260px; green / cobalt / amber): numeral 01–03 (Bricolage 56px) + title 26px + body — "Plain English or nothing" / "Doing beats knowing" / "No shame in the syllabus".
4. **Team quote** — centered blockquote "We're not here to make you feel behind. We're here to make catching up feel easy." — The Self Made School Team.
...
- Grain: inline data-URI SVG (`feTurbulence fractalNoise baseFrequency 0.9`, 160×160 tile) — in the files.
