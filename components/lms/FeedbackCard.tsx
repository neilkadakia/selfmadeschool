...
export default function FeedbackCard({ totalDone }: { totalDone: number }) {
  const lms = useLms();
  ...
  const submit = async (e: FormEvent) => {
    ...
    const res = await apiFeedbackSubmit(
      lms.auth!.token,
      text.trim(),
      `${totalDone} units in · The 13th Grade`
    );
    if (res.ok) {
      setStatus("done");
      lms.feedbackDone();
  ...
  if (status === "done") {
    return (
      <section className="lms-feedback" aria-label="Feedback">
        <p className="lms-feedback-thanks">Noted, and thank you. If it lands on the homepage, that&apos;s your quote out there.</p>
      </section>
    );
  }
  ...
  return (
    <section className="lms-feedback" aria-label="Feedback">
      ...
      <p className="lms-section-sub">
        {totalDone} units in — what would you tell a friend about this course? The best lines go on
        the homepage (with your first name, after review). Honest beats flattering.
      </p>
